...
async function syncGlpiFromCsv() {
    ...
    for (const csvRow of csvComputers) {
        ...
        let role = ComputerRole.USER;
        if ((typeof glpiComp.operatingsystems_id === 'string' && glpiComp.operatingsystems_id.toLowerCase().includes('server')) ||
        ...
        await prisma.computer.upsert({
            ...
            create: {
                ...
                role,
                networkInterfaces: {
                    create: glpiComp._networks?.map((n) => ({
                        name: n.name,
                        ipAddress: n.ip,
                        macAddress: n.mac
                    })) || []
