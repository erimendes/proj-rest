...
// --- 2. FUNÇÃO PRINCIPAL DE SINCRONIZAÇÃO DE RAM ---
async function syncRAMMassivo() {
    ...
    try {
        ...
        for (const target of targets) {
            ...
            try {
                // Busca pentes de memória no GLPI
                ...
                if (Array.isArray(ramItems) && ramItems.length > 0) {
                    // Soma todos os pentes (campo 'size' em MB)
                    const totalRamMB = ramItems.reduce((acc: number, item: any) => {
                ...
                // Usamos o glpiId como âncora principal para o update
