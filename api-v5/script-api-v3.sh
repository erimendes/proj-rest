...
export class RolesGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    return requiredRoles.includes(user.role); // Verifica se o papel do usuário está entre os papéis permitidos
    // return requiredRoles.some(role => user.roles.includes(role)); // Verifica se o papel do usuário está entre os papéis permitidos
  }
}
EOF
